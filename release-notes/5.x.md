...
## Send message to a queue with retries

A new function HmppsQueue.sendMessage() is a wrapper for the AWS SqsAsyncClient.sendMessage().
It retries the call when an error occurs using a flexible retry policy in the same way as the existing wrapper HmppsTopic.publish().
...
- software.amazon.awssdk:bom [2.36.3 -> 2.39.2]
- com.microsoft.azure:applicationinsights-core [3.7.5 -> 3.7.6]
- org.springframework.boot:spring-boot-dependencies [3.5.7 -> 3.5.8]
 - io.awspring.cloud:spring-cloud-aws-sns [3.4.0 -> 3.4.1]
 - io.awspring.cloud:spring-cloud-aws-sqs [3.4.0 -> 3.4.1]
 - io.awspring.cloud:spring-cloud-aws-starter [3.4.0 -> 3.4.1]
...
- software.amazon.awssdk:bom [2.33.12 -> 2.36.3]
- com.microsoft.azure:applicationinsights-core [3.7.4 -> 3.7.5]
- org.springframework.boot:spring-boot-dependencies [3.5.6 -> 3.5.7]

## CVEs

- suppress CVE-2014-5118, CVE-2025-21199

# 5.6.0

## Publish telemetry when message moved to a DLQ

If there is an Application Insights telemetry client bean available, publish telemetry when moving a message to the DLQ.

Telemetry can be found with Log Analytics query:
```KQL
AppEvents
...
## Support custom error visibility timeouts

Support the ability to define a list of error visibility timeouts at the project, queue and event level to allow retrying failed messages for longer before they are sent to the DLQ.
