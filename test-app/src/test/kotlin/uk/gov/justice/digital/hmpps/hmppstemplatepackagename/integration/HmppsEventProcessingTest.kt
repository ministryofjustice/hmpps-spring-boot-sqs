...
class HmppsEventProcessingTest : IntegrationTestBase() {
  ...
  fun `event is moved to the dead letter queue when an exception is thrown`() {
    ...
    await.atMost(Duration.ofMinutes(5L)) untilCallTo { inboundSqsDlqClient.countMessagesOnQueue(inboundDlqUrl).get() } matches { it == 1 }
    assertThat(inboundSqsClient.countMessagesOnQueue(inboundQueueUrl).get()).isEqualTo(0)
  }
}
